...
import (
	...
	"github.com/xordataexchange/crypt/backend/zk"
...
// NewStandardZKConfigManager returns a new ConfigManager backed by consul.
func NewStandardZKConfigManager(machines []string) (ConfigManager, error) {
	store, err := zk.New(machines)
	if err != nil {
		return nil, err
	}
	return NewStandardConfigManager(store)
...
// NewZKConfigManager returns a new ConfigManager backed by consul.
// Data will be encrypted.
func NewZKConfigManager(machines []string, keystore io.Reader) (ConfigManager, error) {
	store, err := zk.New(machines)
	if err != nil {
		return nil, err
	}
